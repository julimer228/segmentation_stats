...
from src.metrics.bfscore import bfscore

"""
Code based on https://github.com/vqdang/hover_net/blob/master/metrics/stats_utils.py
"""
...
def get_fast_detection_stats(true, pred, match_iou=0.5):
    """`match_iou` is the IoU threshold level to determine the pairing between
    GT instances `p` and prediction instances `g`. `p` and `g` is a pair
    if IoU > `match_iou`. However, pair of `p` and `g` must be unique
    (1 prediction instance to 1 GT instance mapping).
    ...
    fdr = fp / (fp + tp)
    return recall, precision, fdr, f1, tp, fp, fn
...
def run_nuclei_inst_stat(pred_dir, true_dir, bf_img_dir, match_iou=0.5):
    file_list = glob.glob("%s/*%s" % (pred_dir, ".npy"))
    file_list.sort()  # ensure same order

    metrics = []
    for filename in tqdm(file_list[:], total=len(file_list[:]), desc="Calculating stats ..."):
        filename = os.path.basename(filename)
        basename = filename.split(".")[0]

        true = np.load(os.path.join(true_dir, basename + ".npy"))
        ...
        pq_info = get_fast_pq(true, pred, match_iou=match_iou)[0]
        det_info = get_fast_detection_stats(true, pred, match_iou=match_iou)
        bf_score, _, img_bf = bfscore(true.copy(), pred.copy(), 2)
        res_dict = {"name": basename,
        ...
        cv2.imwrite(os.path.join(str(bf_img_dir), basename + ".png"), img_bf)
        metrics.append(res_dict)
    ####
    res = pd.DataFrame(metrics)

    summary = res.select_dtypes(include='number').agg(['mean', 'median', 'std']).transpose()
    TP_all = res["TP"].sum()
    FP_all = res["FP"].sum()
    FN_all = res["FN"].sum()
    recall_all = TP_all/(FN_all+TP_all)
    precision_all = TP_all/(FP_all+TP_all)
    ...
    return res, summary, summary_detection


def save_prediction_visualization(gt_dir, pred_dir, res_dir):
    gt_paths = sorted(Path(gt_dir).glob('*.*'))
    pred_paths = sorted(Path(pred_dir).glob('*.*'))

    Y_gt = list(map(lambda path: np.load(path), gt_paths))
    Y_pred = list(map(lambda path: np.load(path), pred_paths))
    names = [n.split('.')[0] for n in os.listdir(gt_dir)]

    # Create images with predictions
    for y_gt, y_pred, n in tqdm(zip(Y_gt, Y_pred, names), total=len(Y_gt), desc="Saving visualization masks ..."):
        # do visualization (pixel level)
        TP = np.logical_and(y_gt, y_pred).astype(int)
        FP = np.logical_and(np.logical_not(y_gt), y_pred).astype(int)
        FN = np.logical_and(np.logical_not(y_pred), y_gt).astype(int)

        res_viz = np.stack([FN, TP, FP], axis=-1)
        res_viz = np.uint8(res_viz * 255)
        cv2.imwrite(os.path.join(str(res_dir), n + ".png"), res_viz)
